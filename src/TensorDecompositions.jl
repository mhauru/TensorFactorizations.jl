...
"""
tensorsplit calls tensorsvd with the arguments given to it to decompose the
given tensor. It then splits the diagonal matrix of singular values into two
with a square root and multiplies these weights into the isometric tensors.
Thus it ends up splitting the given tensor into two parts, which are then
returned, possibly together with auxiliary data such as a truncation error. If
the keyword argument hermitian=true, an eigenvalue decomposition is used in
stead of an SVD.
...
Transpose indices a to be on one side of A, b on the other, and reshape A to a
matrix. Then find the eigenvalues E[i] and eigenvectors U[:,i] of this matrix.
Finally, reshape U to a tensor that has a new index coming from the
decomposition as its last one and indices a as its first indices.

Truncation and error printing work as with tensorsvd.

Note that no iterative techniques are used, meaning choosing to truncate
provides no performance benefits: The full eigenvalue decomposition is computed
in any case.

The keyword argument hermitian (false by default) tells the algorithm whether
the reshaped matrix is Hermitian or not. If hermitian=true, then
A = U*diagm(E)*U' up to the truncation error.

Output is E, U, and possibly error, if return_error=true. Here E is a vector of
eigenvalues values and U[:,...,:,k] is the kth eigenvector.
"""
...
"""
    tensorsvd(A, a, b;
              ...
              norm_type=:frobenius)
Transpose indices a to be on one side of A, b on the other, and reshape A to a
matrix. Then singular value decompose this matrix into U*diagm(S)*Vt.  Finally,
reshape the unitary matrices U and Vt to tensors that have a new index coming
from the SVD, for U as the last index and for Vt as the first, and U to have
indices a as its first indices and V to have indices b as its last
indices.

If eps>0 then the SVD may be truncated if the relative error can be kept below
eps. For this purpose different dimensions to truncate to can be tried, and
these dimensions should be listed in chis. If chis is nothing (the default)
then the full range of possible dimensions is tried. If break_degenerate=false
(the default) then the truncation never cuts between degenerate singular
values. degeneracy_eps controls how close the values need to be to be
...
difference between A and the decomposition, divided by the Frobenius norm of A.
This is the same thing as the square-sum of the singular values that are
truncated out, divided by the square-sum of all the singular values. The other
option is :trace, in which case a regular sum of absolute values is used in
stead of a square-sum.

If print_error is true the truncation error is printed. The default is false.

If return_error is true then the relative truncation error is also returned.
The default is false.

Note that no iterative techniques are used, meaning choosing to truncate
provides no performance benefits: The full SVD is computed in any case.

Output is U, S, Vt, and possibly error. Here S is a vector of singular values
and U and Vt are isometric tensors (unitary if the matrix that is SVDed is
square and there is no truncation) such that  U*diag(S)*Vt = A, up to
truncation errors.
"""
